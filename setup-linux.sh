...
if [[ "${OS_TYPE}" == "arch" ]]; then
	...
	sudo pacman -Sy --noconfirm flatpak mono sdl12-compat sdl_image sdl wine winetricks

elif [[ "${OS_TYPE}" =~ "debian" ]]; then
	echo "[INFO] Installing prerequisite packages for Debian-like systems"
	sudo apt-get install -y flatpak sdl12-compat sdl_image sdl wine winetricks
fi
